...
format_namenode(){
    if [[ $1 == "clusterid" ]]; then
        /usr/local/hadoop/bin/hdfs namenode -format -clusterId $2
    elif [[ $1 == "normal" ]]; then
...
fix(){
    ...
    echo "Melakukan cek clusterID pada namenode dan datanode..."
    is_cid_same
    if [[ $is_cid_datanode_same == true && $is_cid_name_and_data_same == false ]]; then
        echo -e "clusterID pada datanode sama.\nclusterID pada namenode tidak sama dengan datanode.\nMemformat namenode dengan clusterID datanode"
